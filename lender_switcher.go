...
fmt.Println("Hello, World!")
	...
	col2 := shim.Column{Value: &shim.Column_String_{String_: city}}
	col3 := shim.Column{Value: &shim.Column_String_{String_: zip}}
	
	colum = append(colum, col1)
	colum = append(colum, col2)
	colum = append(colum, col3)
fmt.Println("Hello, World!!!")
	row, err := stub.GetRow("TitleInfo", colum)
	fmt.Println("Hello, World!")
	fmt.Println(err)
	if err != nil {
	...
	fmt.Println("number of columsn :!!!!!!!!!!!!", len(row.Columns))
	res2E :=  TitleInfo{}
	res2E.Name = row.Columns[0].GetString_()
	res2E.Address = row.Columns[1].GetString_()
	res2E.City = row.Columns[2].GetString_()
	res2E.State = row.Columns[3].GetString_()
	res2E.Zip = row.Columns[4].GetString_()
	res2E.Info = row.Columns[5].GetString_()
